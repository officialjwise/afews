...
const PhoneInput = React.forwardRef<HTMLInputElement, PhoneInputProps>(
  ({ value, onChange, className, placeholder, defaultCountry = "GH", disabled }, ref) => {
    ...
    // Put default country first when not searching
    const sortedCountries = search.trim()
      ? filteredCountries
      : [
          ...filteredCountries.filter((c) => c.code === defaultCountry),
          ...filteredCountries.filter((c) => c.code !== defaultCountry),
        ];
          ...
          <div className="absolute z-50 mt-1 w-full rounded-md border border-border bg-popover shadow-md">
            ...
            <div className="max-h-52 overflow-auto">
              {sortedCountries.length === 0 ? (
                <p className="px-3 py-3 text-sm text-muted-foreground text-center">No countries found</p>
              ) : (
