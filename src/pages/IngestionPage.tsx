...
import { CloudRain, Database, Globe, Layers, Play, RefreshCw, AlertTriangle, CheckCircle2, Clock } from "lucide-react";
...
const DATA_SOURCES = [
  {
    ...
    status: "healthy",
    icon: CloudRain,
    description: "Global weather forecast data including precipitation, temperature, and wind."
  },
  {
    id: "chirps",
    name: "CHIRPS",
    type: "Historical Rainfall",
    frequency: "Daily",
    lastRun: "1 day ago",
    status: "healthy",
    icon: Database,
    description: "Climate Hazards Group InfraRed Precipitation with Station data (30+ year history)."
  },
  {
    id: "copernicus-dem",
    name: "Copernicus DEM",
    ...
    status: "healthy",
    icon: Layers,
    description: "High-resolution digital elevation model (GLO-30) for terrain analysis."
  },
  {
    id: "osm",
    name: "OpenStreetMap",
    ...
    status: "warning",
    icon: Globe,
    description: "Building footprints, roads, and critical infrastructure data via Overpass API."
  }
];

const PIPELINE_LOGS = [
  { id: 1, source: "Open-Meteo", action: "Fetch Forecast", status: "success", timestamp: "2023-11-15 08:00:00", duration: "45s", details: "Retrieved 450MB of GRIB2 data" },
  ...
  { id: 4, source: "OSM", action: "Area Update (Lekki)", status: "failed", timestamp: "2023-11-10 14:30:00", duration: "300s", details: "Timeout connecting to Overpass API" },
  { id: 5, source: "Copernicus DEM", action: "Tile Gen", status: "success", timestamp: "2023-10-15 09:00:00", duration: "15m", details: "Generated 50 tiles for Lagos" },
];
...
export default function IngestionPage() {
  ...
  const handleTrigger = async (sourceId: string) => {
    ...
    try {
      ...
      // Simulate delay for effect since API might be mocked or fast
  ...
  return (
    <div className="p-6 space-y-6 max-w-7xl">
      <div className="flex items-start justify-between gap-4">
        ...
        <Button variant="outline" size="sm" onClick={() => toast.success("Refreshed status")}>
            ...
            <CardContent>
              <div className="text-2xl font-bold flex items-center gap-2">
                {source.status === "healthy" ? (
                  <span className="inline-flex h-2.5 w-2.5 rounded-full bg-status-active" />
                ) : (
                  <span className="inline-flex h-2.5 w-2.5 rounded-full bg-status-warning" />
                )}
      ...
      <Tabs defaultValue="sources" className="space-y-4">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-2">
                      ...
                      <span className="text-[10px] text-muted-foreground flex items-center gap-1">
                        <Clock className="h-3 w-3" /> {source.frequency}
                      </span>
                    </div>
                  </div>
                </div>
              ...
              <tbody>
                {PIPELINE_LOGS.map((log) => (
                  <tr key={log.id} className="border-b last:border-0 hover:bg-muted/50 transition-colors">
                    ...
                    <td className="p-4 align-middle font-medium">{log.source}</td>
                    <td className="p-4 align-middle">
                      <div>{log.action}</div>
                      <div className="text-xs text-muted-foreground">{log.details}</div>
                    </td>
                    <td className="p-4 align-middle text-muted-foreground font-mono text-xs">{log.duration}</td>
                    <td className="p-4 align-middle text-muted-foreground text-xs">{log.timestamp}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <TabsContent value="config">
          <div className="panel p-8 text-center text-muted-foreground">
             <p>Configuration settings are managed in System Settings.</p>
             <Button variant="link" className="mt-2" onClick={() => window.location.href = '/admin/settings'}>Go to Settings</Button>
          </div>
        </TabsContent>
      </Tabs>
    </div>
