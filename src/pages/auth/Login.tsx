...
export default function Login() {
  ...
  const handleRequestOtp = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    try {
      const res = await authApi.loginOtpRequest(email);
    ...
    try {
      ...
      if (err instanceof ApiError) {
        if (err.status === 401 || err.status === 403) {
          setError("Invalid credentials or verification code. Please try again.");
  ...
  }, [email, password, otpCode, login, navigate]);
          ...
          {step === "email" ? (
            ...
            <form onSubmit={handleLogin} className="space-y-4">
              <p className="text-sm text-muted-foreground rounded-md bg-muted px-3 py-2">
                {phoneMasked
                  ? <>A 6-digit code was sent to <span className="font-medium text-foreground">{phoneMasked}</span>.</>
                  : <>A 6-digit code was sent to the phone linked to <span className="font-medium text-foreground">{email}</span>.</>}
              </p>
